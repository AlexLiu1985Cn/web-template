namespace CmsApp20.CmsBack
{
    ...
    public class TemplateMng_Mobile : Page
    {
        private string[] astrTitle = new string[] { "<a href=\"TemplateMng_Mobile.aspx?Mdl=0\">首页模板</a>", "<a href=\"TemplateMng_Mobile.aspx?Mdl=1\" > - 简介模块模板</a>", "<a href=\"TemplateMng_Mobile.aspx?Mdl=2\" > - 新闻模块模板</a>", "<a href=\"TemplateMng_Mobile.aspx?Mdl=3\" > - 产品模块模板</a>", "<a href=\"TemplateMng_Mobile.aspx?Mdl=4\" > - 下载模块模板</a>", "<a href=\"TemplateMng_Mobile.aspx?Mdl=5\" > - 图片模块模板</a>", "<a href=\"TemplateMng_Mobile.aspx?Mdl=6\" > - 留言模块模板</a>", "<a href=\"TemplateMng_Mobile.aspx?Mdl=8\" > - 招聘模块模板</a>" };
        ...
        {
            base.Server.Transfer("ModuleTmplt_Mobile.aspx?Mdl=" + base.Request.QueryString["Mdl"]);
        }
        ...
        private void DataToBind()
        {
            ...
            this.Bll1.GetTmpltFileName_mobile(this.strMdl, out str);
        ...
        private bool ReadTemplate()
        {
            ...
            string str2 = this.Bll1.GetTmpltFileName_mobile(this.strMdl, out str);
            string str3 = Convert.ToString(this.Bll1.DAL1.ExecuteScalar("select TemplatePath from TemplateInfo where IsUsed=true and IsMobile=true"));
            this.ViewState["TPath"] = str3 + "/" + str2;
            this.strMdlFName = str + "模板：" + str3 + "/" + str2;
            string path = base.Server.MapPath(@"..\template\" + str3 + "/");
            if (!Directory.Exists(path))
            {
                this.Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "Dir", "<script>alert('模板路径/template/" + str3 + "/不存在！');</script>");
            ...
            if (!File.Exists(str5))
            {
                this.Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "File", "<script>var con=confirm('" + str + "模板文件/template/" + str3 + "/" + str2 + "不存在！是否生成空白模板？');if(con==true){$('HdnPath').value='" + str3 + "/" + str2 + "';$('BtnCreateEmp').click();} else history.back();</script>");
                return false;
            }
            try
            {
            ...
            {
                this.Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "File", "<script>alert('未能读取" + str + "模板文件/template/" + str3 + "/" + str2 + "！');</script>");
                base.Response.Write("未能读取" + str + "模板文件/template/" + str3 + "/" + str2 + "！");
                this.BtnSaveList.Visible = false;
                this.BtnSaveLM.Visible = false;
                this.BtnSaveClass.Visible = false;
                return false;
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("<!DOCTYPE html PUBLIC \"-//WAPFORUM//DTD XHTML Mobile 1.0//EN\" \"http://www.wapforum.org/DTD/xhtml-mobile10.dtd\">\n");
            builder.Append("<html xmlns=\"http://www.w3.org/1999/xhtml\">\n");
            builder.Append("<head>\n");
            builder.Append("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" />\n");
            builder.Append("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, user-scalable=0, minimum-scale=1.0, maximum-scale=1.0\">\n");
            builder.Append("<meta name=\"apple-mobile-web-app-capable\" content=\"yes\">\n");
            builder.Append("<meta name=\"apple-mobile-web-app-status-bar-style\" content=\"black\">\n");
            builder.Append("<meta content=\"telephone=no\" name=\"format-detection\" />\n");
