namespace CmsApp20.CmsBack
{
    ...
    public class QH_AdvertiseManage : Page
    {
        ...
        private ICollection CreateSource()
        {
            ...
            try
            {
                set = this.Bll1.DAL1.GetDataTablePage(nStart, this.PageSize, "select id ,order,title,linkUrl,ImageUrl,switch(LinkType='0','文字链接',LinkType='1','图片链接',LinkType='2','Js代码') as LinkType from QH_Advertise order by CLNG(order) desc");
            }
            catch (Exception exception)
            {
                flag = false;
                SystemError.CreateErrorLog("读广告管理错误： " + exception.ToString());
            }
            if (flag)
            {
                return set.Tables[0].DefaultView;
            }
            base.Response.Write("<script>alert('读广告管理数据库失败，详细信息请查看错误日志！');</script>");
            return null;
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Commen1.JudgeLogin(this.Page))
            {
                ...
                if (!base.IsPostBack)
                {
                    ...
                    this.RecordCount = this.Bll1.DAL1.ExecuteReader("select count(*) as co from QH_Advertise ");
